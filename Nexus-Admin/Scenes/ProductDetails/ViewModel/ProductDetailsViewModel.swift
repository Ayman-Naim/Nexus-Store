...
//        }
//    }
    private var currentVariant: Int? {
        didSet {
//            guard let currentVariant = currentVariant else { return }
//            currentQuantity = currentVariant.inventoryQuantity
//            DispatchQueue.main.async {
...
//        cell.setProductImage(withURLString: images[index].src)
    ...
    func fetchProduct(id: Int) {
        loading?(true)
//        AF.request(K.baseURL + "/products/\(id).json", headers: K.APIHeader).responseDecodable(of: ProductResponse.self) { [weak self] response in

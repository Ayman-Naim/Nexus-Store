...
class WishListService {
    ...
    func add(productID: Int, toCustomer customerID: Int, completion: @escaping (Error?) -> Void) {
    ...
    func remove(productID: Int, fromCustomer customerID: Int, completion: @escaping (Error?) -> Void) {
        let url = customersEndpoint + "/\(customerID)/metafields.json?key=\(productID)"
        AF.request(url, method: .post, headers: header).responseDecodable(of: Metafields.self) { response in
            switch response.result {
            case .success(_):
                completion(nil)
            case .failure(let error):
    ...
    private func fetchAllMetafields(forCustom customerID: Int, completion: @escaping (Result<[Int], Error>) -> Void) {
        let url = customersEndpoint + "/\(customerID)/metafields.json"
        AF.request(url, method: .get, headers: header).responseDecodable(of: Metafields.self) { response in
            ...
            case .success(let data):
                ...
                let productIDs = metafields.compactMap({ Int($0.key) })
    ...
    func getAllProducts(forCustom customerID: Int, completion: @escaping (Result<[Product], Error>) -> Void) {
        fetchAllMetafields(forCustom: customerID) { result in
            switch result {
            case .success(let productIDs):
                let stringIDs = productIDs.compactMap( { String($0) }).joined(separator: ",")
